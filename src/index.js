...
app.listen(app.get("port"), () => {
  ...
  app.post('/src/app/routes/form.js', (req1, res1) => {
    ...
    const {identific,nombre, apellido_uno, apellido_dos, sexo, edad, correo, ndli} = req1.body;
    ...
    // ****** PROBANDO NO CREO QUE SE NECESARIA **************
 ...
 /*  app.get("/verific.js", (req, res) => {     // nComentado para verificar q no se muestre Resultado al Inicio
    res.render("login");
  })
 */
...
let mostrar_pantalla_juego_términado = false;   // Probando -  true
  ...
  select_id("btn3").style.display = "none";
    ...
    while (npreguntas.includes(n)) {
      ...
      if (npreguntas.length == base_preguntas.length) {
        let mostrar_pantalla_juego_términado = true;   // probando aqui...nueva linea - perfil inicio
        //Aquí es donde el Test se reinicia
        ...
        if (mostrar_pantalla_juego_términado) {
          ...
          //let perfilCompleto; 
          //let categoriaCompleta;
          if ((arte_creatividad==0) && (ciencias_sociales==0) && (ciencias_eaf==0) && (ciencia_tecnologia==0) && (ciencia_ebs==0)){
            console.log("Variables en 0...")
          }
          else{
          ...
          if (perfilCompleto != null) {
            app.post("/", (req, res) => {
              ...
              conection.query(consulta3, (err, result)=>{
                if (err == null){
                  console.log("Verificando los Nombres de Categorias");
